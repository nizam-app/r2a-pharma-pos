...
/** JWT tenancy + display names. storeId is read-only (no branch switch). */
...
export async function fetchTenantContext(): Promise<TenantContextPayload> {
  return apiRequest<TenantContextPayload>("/api/v1/tenant/context");
}
